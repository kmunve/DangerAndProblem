    ...
    for a in kdv['d']['results']:
        try:
            if 'AvalCauseKDV' in url and a['ID'] > 9 and a['ID'] < 26:      # this table gets special treatment
                dict[a["ID"]] = remove_norwegian_letters(a["Description"])
            else:
                dict[a["ID"]] = remove_norwegian_letters(a["Name"])
...
def get_forecast_region_name(region_id):
    ...
    forecast_region_tid = region_id
    forecast_region_name = forecast_region_kdv[forecast_region_tid]
    forecast_region_name = remove_norwegian_letters(forecast_region_name)
...
def get_problems_from_AvalancheProblemV(region_name, region_id, start_date, end_date):
    ...
    odata_query = add_norwegian_letters(odata_query)
    ...
    if len(result) != 0:
        for p in result:
            ...
            if cause_name1 != "Ikke gitt":
                prob = ap.AvalancheProblem(region_id, region_name, date, 0, cause_name1, source)
            ...
            if cause_name2 != "Ikke gitt":
                prob = ap.AvalancheProblem(region_id, region_name, date, 1, cause_name2, source)
                prob.set_regobs_view(view)
                prob.set_regid(p['RegID'])
                prob.set_url("{0}{1}".format(registration_basestring, prob.regid))
                problems.append(prob)
            ...
            if cause_name3 != "Ikke gitt":
                prob = ap.AvalancheProblem(region_id, region_name, date, 2, cause_name3, source)
                prob.set_regobs_view(view)
                prob.set_regid(p['RegID'])
                prob.set_url("{0}{1}".format(registration_basestring, prob.regid))
                problems.append(prob)
    ...
    if len(result) != 0:
        for p in result:
            ...
            if cause != 0 and cause_ext != 0:
                ...
                cause_name = remove_norwegian_letters(p['AvalCauseName'])
                cause_ext_name = remove_norwegian_letters(p['AvalCauseExtName'])
                cause_name = "{0}, {1}".format(cause_name, cause_ext_name)
                source = "Observasjon"

                prob = ap.AvalancheProblem(region_id, region_name, date, order, cause_name, source)
...
def get_problems_from_AvalancheEvalProblem2V(region_name, region_id, start_date, end_date):
    '''
    # Used from 2014-02-10 upp to today
    ...
    aval_cause_kdv = get_kdv('AvalCauseKDV')
    ...
    if len(result) != 0:
        for p in result:
            ...
            if cause != 0:
                ...
                cause_name = aval_cause_kdv[cause_tid]
                source = "Observasjon"
...
def get_problems_from_AvalancheWarningV(forecast_region_name, region_id, start_date, end_date):
    '''

    :param forecast_region_name:
    :param region_id:
    :param start_date:
    :param end_date:
    :return:
    ...
    odata_query = "DtObsTime gt datetime'{1}' and " \
                 "DtObsTime lt datetime'{2}' and " \
                 "ForecastRegionName eq '{0}' and " \
                 "LangKey eq 1".format(forecast_region_name, start_date, end_date)
    odata_query = add_norwegian_letters(odata_query)
    ...
    if len(result) != 0:
        for p in result:
            ...
            if cause_name1 != "Ikke gitt":
                prob = ap.AvalancheProblem(region_id, forecast_region_name, date, 0, cause_name1, source)
                prob.set_regobs_view(view)
                prob.set_regid(p['RegID'])
            ...
            if cause_name2 != "Ikke gitt":
                prob = ap.AvalancheProblem(region_id, forecast_region_name, date, 1, cause_name2, source)
                prob.set_regobs_view(view)
                prob.set_regid(p['RegID'])
                prob.set_url(view_url_base.format(p['RegID']))
                problems.append(prob)
            ...
            if cause_name3 != "Ikke gitt":
                prob = ap.AvalancheProblem(region_id, forecast_region_name, date, 2, cause_name3, source)
                prob.set_regobs_view(view)
                prob.set_regid(p['RegID'])
                prob.set_url(view_url_base.format(p['RegID']))
                problems.append(prob)
...
def get_problems_from_AvalancheWarnProblemV(region_name, region_id, start_date, end_date):
    '''
    AvalancheWarnProblemV used from 2012-11-15 to today. There was made changes to the view in des 2013 wich i thinc affected destructive
    size and avalanche cause. Changes were made to the datamodel before startup in december 2014. Changes ionvolve use
    of cause_name and avalanche size.
    ...
    if len(result) != 0:
        for p in result:
            ...
            if regid in valid_regids:
                ...
                aval_size = remove_norwegian_letters(p['DestructiveSizeExtName'])
                aval_type = p['AvalancheExtName']
                aval_trigger = remove_norwegian_letters(p['AvalTriggerSimpleName'])
                aval_probability = remove_norwegian_letters(p['AvalProbabilityName'])
                aval_distribution = remove_norwegian_letters(p['AvalPropagationName'])
                ...
                if date > datetime.datetime.strptime('2013-11-15', '%Y-%m-%d').date():
                    ...
                    cause_name = aval_cause_kdv[aval_cause_tid]
                    aval_size = remove_norwegian_letters(p['DestructiveSizeName'])
                    aval_probability = remove_norwegian_letters(p['AvalProbabilityName'])
                    aval_distribution = remove_norwegian_letters(p['AvalPropagationName'])
...
def get_all_problems_with_date(region_id, start_date, end_date):
    '''
    Bug i metoden med fra og med/ til og med. Virker som varsler henter for 101 dager men kan bare se at varlingsapi
    leverer <start,end]. Virker fint om man kjører hele sesonger
    ...
    request_from_date = datetime.datetime.strptime(start_date, "%Y-%m-%d")
    request_to_date =  datetime.datetime.strptime(end_date, "%Y-%m-%d")
    ...
    if request_to_date - request_from_date > datetime.timedelta(days=200):

        variable_start = request_from_date
        while variable_start < request_to_date:

            variable_end = variable_start + datetime.timedelta(days=200)
            if variable_end > request_to_date:
                variable_end = request_to_date

            start = variable_start.strftime("%Y-%m-%d")
            end = variable_end.strftime("%Y-%m-%d")

            problem_v = problem_v + get_problems_from_AvalancheProblemV(region_name, region_id, start, end)
            eval_problem_v = eval_problem_v + get_problems_from_AvalancheEvalProblemV(region_name, region_id, start, end)
            eval_problem2_v = eval_problem2_v + get_problems_from_AvalancheEvalProblem2V(region_name, region_id, start, end)
            warning_v = warning_v + get_problems_from_AvalancheWarningV(region_name, region_id, start, end)
            warn_problem_v = warn_problem_v + get_problems_from_AvalancheWarnProblemV(region_name, region_id, start, end)

            variable_start = variable_start + datetime.timedelta(days=200)
    ...
    problems = problem_v + eval_problem_v + eval_problem2_v \
               + warning_v + warn_problem_v
...
def get_all_problems(region_id):
    ...
    Merk at odata har begrensining på 1000 elementer. Det er flere så kommer de ikke med.
    ...
    for d in dates:
        ...
        problems = problems + problem_v + eval_problem_v + eval_problem2_v \
               + warning_v + warn_problem_v

    return problems

def get_avalanche_danger(region_id, start_date, end_date):
    """

    forecast_region_name = get_forecast_region_name(region_id)
    ...
    oDataQuery = add_norwegian_letters(oDataQuery)    # Need norwegian letters in the URL

    print("Getting AvalancheEvaluations for {0} from {1} til {2}").format(forecast_region_name, start_date, end_date)

    print("..from AvalancheEvaluation3V")
    url = "http://api.nve.no/hydrology/regobs/{0}/Odata.svc/AvalancheEvaluation3V?$filter={1}&$format=json".decode('utf8').format(apiVersion, oDataQuery)
    ...
    url = "http://api.nve.no/hydrology/regobs/{0}/Odata.svc/AvalancheEvaluation2V?$filter={1}&$format=json".decode('utf8').format(apiVersion, oDataQuery)
    ...
    url = "http://api.nve.no/hydrology/regobs/{0}/Odata.svc/AvalancheEvaluationV?$filter={1}&$format=json".decode('utf8').format(apiVersion, oDataQuery)
    ...
    print("Getting AvalancheWarnings for {0} from {1} til {2}").format(forecast_region_name, start_date, end_date)
    # from AvalancheWarningV
    # from AvalancheWaring2V
    ...
    if len(avalEval3) != 0:
        for e in avalEval3:
            ...
            dangerlevel = e['AvalancheDangerTID']
            nick = e['NickName']
            eval = AvalancheEvaluation(region_id, forecast_region_name, "AvalancheEvaluation3V", date, dangerlevel, nick)
    ...
    if len(avalEval2) != 0:
        for e in avalEval2:
            ...
            dangerlevel = e['AvalancheDangerTID']
            nick = e['NickName']
            eval = AvalancheEvaluation(region_id, ForecastRegionName, "AvalancheEvaluation2V", date, dangerlevel, nick)
            evaluations.append(eval)
    ...
    if len(avalEval) != 0:
        for e in avalEval:
            ...
            dangerlevel = e['AvalancheDangerTID']
            nick = e['NickName']
            eval = AvalancheEvaluation(region_id, ForecastRegionName, "AvalancheEvaluationV", date, dangerlevel, nick)
            evaluations.append(eval)
...
def __get_cause_from_old_cause(old_cause_parameter_name, old_cause_tid):
    ...
    if old_cause_parameter_name == 'AvalancheProblem':
        # avalanche_problem_kdv = get_kdv('AvalancheProblemKDV')
        if old_cause_tid == 0:
            return aval_cause_kdv[0]
        elif old_cause_tid == 1:
            return aval_cause_kdv[28]
...
if __name__ == "__main__":
    ...
    region_list = get_kdv("ForecastRegionKDV")
